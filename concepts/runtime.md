...
对于分布式执行，Flink将多个 operator 的 subtask *链化(chains)* 成为 *tasks*。 每个task由一个线程执行。
将算子链化合并成任务是一个很有用的优化：它减少了线程间切换和缓冲的开销， 提高了整体吞吐量， 减少时延。
链化操作可配置；详细信息参考[链接文档 (chaining docs)](../dev/stream/)
...
Flink运行包含两种类型进程:
  ...
  - **JobManagers** (也被称作 *masters*) 协同分布式执行节点。分发tasks，协同
    checkpoints，协调故障恢复等。

    系统中至少有一个JobManager。 高可用性设置将包含多个JobManager，其中一个始终是 *leader*，其他人都是 *standby* 状态。

  - **TaskManagers** (也被称作 *workers*) 用于执行数据流的 *tasks* (更具体一点, 是执行 subtasks), 缓冲并改变数据*streams*.
...
宣布自己可用，并由JobManager下发工作。

**client** 不是运行和程序执行的一部分，但是常用于准备数据流并将其发送到JobManager。
完成这些任务后，client 可以中断连接，或者保持连接用于接收运行结果。client 作为触发Java / Scala程序的一部分运行，或者在命令行进程中运行 `./bin/flink` 运行...
...
每个 worker (TaskManager) 是一个 *JVM process*，并可以在单独的线程中执行一个或多个子任务。
为了控制worker能接受多少任务，worker有所谓的 **task slots** （至少一个）。
...
根据经验，默认比较好的 task slot 数量是CPU核数。使用超线程技术时，每个 slot 需要2个或更多的硬件线程上下文环境。

{% top %}

## 状态后端

存储在 key/value 索引的正确数据结构取决于所选择的 [**状态后端 (state backend)**](../ops/state_backends.md)。一个状态后端将数据存储在
内存中的哈希映射，另一个状态后端用 [RocksDB](http://rocksdb.org) 作为 key/value 存储。
...
用Data Stream API 编写的程序可以从 **保存点(Savepoints)** 恢复执行。savepoint 允许在不会丢失任何状态情况下，更新程序和Flink群集。 

[Savepoint](../ops) 是 **手动触发的 checkpoint**, 它获取程序快照并写入状态后端。他们依赖常规的 checkpoint 机制。在执行期间，程序会定期在 worker 节点上生成快照并产生 checkpoints。 
为了恢复，仅需要最后完成的 checkpoint，并且一旦新的 checkpoint 完成，可以安全地丢弃旧的 checkpoint。

savepoint 类似于这些定期 checkpoint，除了它们 **由用户触发** 并且在新 checkpoint 完成时 **不会自动失效**。
